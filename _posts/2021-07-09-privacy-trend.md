...
프라이버시에 대해 다루는 2부작 콘텐츠입니다. 이번 글에서는 프라이버시 트렌드와 그 영향력을 짚어봅니다. 다음 글에서는 프라이버시 리스크 관리에 있어서 제품 설계·운영 상의 주의점에 대해 다룹니다.
{: .infor}
